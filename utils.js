...
if (!clientId) {
        log.err(
                "CREDENTIALS",
                `Please provide a valid clientId in file ${path.normalize(global.client.dirConfig)}`,
        );
        process.exit();
}
if (!clientSecret) {
        log.err(
                "CREDENTIALS",
                `Please provide a valid clientSecret in file ${path.normalize(global.client.dirConfig)}`,
        );
        process.exit();
}
if (!refreshToken) {
        log.err(
                "CREDENTIALS",
                `Please provide a valid refreshToken in file ${path.normalize(global.client.dirConfig)}`,
        );
        process.exit();
}
...
function getPrefix(threadID) {
        if (!threadID || isNaN(threadID))
                throw new Error(
                        "The first argument (threadID) must be a number",
                );
...
function message(api, event) {
        async function sendMessageError(err) {
                if (typeof err === "object" && !err.stack)
                        ...
                        err = utils.removeHomeDir(
                                `${err.name || err.error}: ${err.message}`,
                        );
...
function removeHomeDir(fullPath) {
        if (!fullPath || typeof fullPath !== "string")
                throw new Error(
                        "The first argument (fullPath) must be a string",
                );
...
async function findUid(link) {
        try {
                ...
                if (isNaN(id)) {
                        ...
                        const el = $('meta[property="al:android:url"]').attr(
                                "content",
                        );
                        if (!el) {
...
async function getStreamFromURL(url = "", pathName = "", options = {}) {
        ...
        try {
                if (!url || typeof url !== "string")
                        throw new Error(
                                `The first argument (url) must be a string`,
                        );
...
async function translate(text, lang) {
        ...
        for (let i = 0; i < text.length; i++) {
                ...
                if (word.indexOf(char) !== -1) {
                        ...
                        const lengWordNoTranslate = wordNoTranslate.length - 1;
                        const twoWordLast =
                                wordNoTranslate[lengWordNoTranslate]?.slice(
                                        -2,
                                ) || "";
        ...
        for (let i = 0; i < wordTransAfter.length; i++) {
                ...
                const numberStartSpace = lengthWhiteSpacesStartLine(
                        wordTranslate[i],
                );
                const numberEndSpace = lengthWhiteSpacesEndLine(
                        wordTranslate[i],
                );
...
const drive = {
        ...
        async deleteFile(id) {
                if (!id || typeof id !== "string")
                        throw new Error(
                                "The first argument (id) must be a string",
                        );
                try {
        ...
        async getFile(id, responseType) {
                ...
                if (typeof responseType !== "string")
                        throw new Error(
                                "The second argument (responseType) must be a string",
                        );
                ...
                const fileName =
                        headersResponse["content-disposition"]
                                ?.split('filename="')[1]
                                ?.split('"')[0] ||
        ...
        async getFileName(id) {
                ...
                const { fileNames: tempFileNames } =
                        global.temp.filesOfGoogleDrive;
                if (tempFileNames[id]) return tempFileNames[id];
                try {
        ...
        async checkAndCreateParentFolder(folderName) {
                if (!folderName || typeof folderName !== "string")
                        throw new Error(
                                "The first argument (folderName) must be a string",
                        );
                let parentID;
                ...
                const parentFolder = findParentFolder.files.find(
                        (i) => i.ownedByMe,
                );
                if (!parentFolder) {
        ...
        constructor(apiKey) {
                ...
                this.api.interceptors.response.use((response) => {
                        return {
                                ...
                                responseHeaders: {
                                        "x-remaining-requests": parseInt(
                                                response.headers[
                                                        "x-remaining-requests"
                                                ],
                                        ),
                                        "x-free-remaining-requests": parseInt(
                                                response.headers[
                                                        "x-free-remaining-requests"
                                                ],
                                        ),
                                        "x-used-requests": parseInt(
                                                response.headers[
                                                        "x-used-requests"
                                                ],
                                        ),
